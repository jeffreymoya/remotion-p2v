...
export class ImageQualityValidator {
  ...
  validateUrlSafety(url: string): boolean {
    try {
      ...
      logger.error(`URL validation error: ${err.message}`);
      return false;
    }
  }
  ...
  async preValidateImage(url: string): Promise<PreValidationResult> {
    ...
    try {
      ...
      const err = error as Error;
      logger.error(`Pre-validation HEAD request failed for ${url}: ${err.message}`);
      return {
        valid: false,
        error: `HEAD request failed: ${err.message}`,
      };
    }
  }
  ...
  async validateImage(
    ...
    try {
      ...
      const err = error as Error;
      logger.error(`Image validation failed for ${imagePath}: ${err.message}`);
      return {
        valid: false,
        error: `Image validation error: ${err.message}`,
      };
    }
  }
