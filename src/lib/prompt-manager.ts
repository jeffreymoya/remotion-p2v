 ...
 */
export function formatKeyValue(obj: Record<string, unknown>, separator: string = ': '): string {
  return Object.entries(obj)
    .map(([key, value]) => `${key}${separator}${String(value)}`)
    .join('\n');
}
...
export class PromptBuilder {
  ...
  addKeyValue(obj: Record<string, unknown>, separator: string = ': '): this {
